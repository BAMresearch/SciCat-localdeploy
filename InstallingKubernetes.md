...
(kubeadm config print init-defaults --component-configs=KubeletConfiguration | sed -e '/ClusterConfiguration/{:a;n;/networking:/{a\  podSubnet: 10.244.0.0/16' -e'};ba' -e '}' | sed -e '/bootstrapTokens/{:a;n;/\(token:\s*\)/d;ba' -e '}' | sed -e 's#\(criSocket:\s*\).*$#\1unix:///var/run/crio/crio.sock#' -e 's#\(advertiseAddress:\s*\).*$#\110.0.9.1#') > "$KUBELET_CFG"
